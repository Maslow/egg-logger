...
class Logger extends Map {
   ...
   */
  log (level, args, meta) {
    // hack log for adding call stack info
    let initiator = 'unknown place'
    try {
      throw new Error()
    } catch (e) {
      const base_dirname = path.dirname(path.dirname(path.dirname(__dirname)))
      initiator = e.stack // Grabs the stack trace
        .split('\n')[3] // Grabs third line
        .trim() // Removes spaces
        .substring(3) // Removes three first characters ("at ")
        .replace(base_dirname, '') // Removes script folder path
    ...
    if (redirectLogger) {
      redirectLogger.log(level, _args, meta)
      return
    }

    for (const transport of this.values()) {
      if (transport.shouldLog(level)) {
        transport.log(level, _args, meta)
      }
    }
  }
